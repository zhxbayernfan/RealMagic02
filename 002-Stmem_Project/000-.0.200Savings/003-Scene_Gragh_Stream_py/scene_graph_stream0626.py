...
class StreamSceneGraph:
    ...
    def run_vlm_descriptions(self, objects, moondream_config):
        ...
        category_prompt = (
            "这是什么物体？只回答一个词：物体类别。\n"
            "示例：椅子、桌子、显示器、瓶子、盒子、墙、门、枕头、植物、杯子、键盘、垃圾桶。\n"
            "只回答类别名称，不要输出其他内容。"
        )
        description_prompt = (
            "用一句话描述这个物体的外观特征以便识别。\n"
            "包含：颜色、材质、独特特征。\n"
            "示例：一把带扶手的黑色皮革办公椅、一个带把手的白色陶瓷杯、一张棕色实木长方形桌子。\n"
            "用20字以内的中文回答。"
        )

        # 与「理解」面板完全对齐：空间去重（同 sentrix-vslam.js DEDUP_M=0.5）
        obj_by_id = {o.id: o for o in objects}
        deduped = []
        DEDUP_M = 0.5

        # 先规范化 center_3d 到 [x, y, z] 列表
        def _norm_center(o):
            c = getattr(o, 'center_3d', None)
            if c is None:
                return None
            if hasattr(c, 'tolist'):
                c = c.tolist()
        ...
        # 保持原始顺序（与 dgsg_objects API 返回顺序一致，前端去重结果才能匹配）
        for obj in objects:
            c3 = _norm_center(obj)
            if c3 is None:
                deduped.append(obj)  # 无法解析坐标的也保留
                continue
            is_dup = False
            for d in deduped:
                dc = _norm_center(d)
                if dc is None:
                    continue
                dx = c3[0] - dc[0]; dy = c3[1] - dc[1]; dz = c3[2] - dc[2]
                if (dx*dx + dy*dy + dz*dz) < DEDUP_M * DEDUP_M:
                    is_dup = True; break
            if not is_dup:
                deduped.append(obj)
        dedup_ids = [str(getattr(o, 'id', '?')) for o in deduped]
        print(f'[VLM] {len(objects)} 个物体 → 空间去重后 {len(deduped)} 个: ids={dedup_ids}')
        ...
        for obj in deduped:
            ...
            if verified is None:
                category = getattr(obj, 'class_name', node.category)  # fallback 到 CLIP 校准后的 YOLO 类名
            ...
            if any(kw == category.lower() for kw in scene_keywords):
                category = getattr(obj, 'class_name', node.category)
            ...
            # Also store on the object so snapshot serialization includes it
            obj.category = category
            obj.description = description
            print(f"  [VLM] obj{obj.id}: category='{category}', desc='{description[:50]}'")

...
def main():
    ...
    # 字数不足550的也重跑
    under_len = set(str(int(r[0])) for r in db.execute("SELECT id FROM memories WHERE LENGTH(description) < 550").fetchall())
    todo = []
    for fname in frames:
        fid = str(int(fname.replace('frame_', '').replace('.jpg', '')))

---
name: 001Stmem-program-logic
description: 数据流完整对照(memory.js↔stmem.html)16个数据物件与ST-N标记索引.
metadata:
  node_type: doc
  type: reference
...
> 基于commit`d15819e`(main首合).

## 1.mood标题:`2026年07月,生活平淡如水`:
**后端memory.js**
|行|内容|
|----|------|
...
## 2.事件数:`28个事件`:
**前端stmem.html**(纯前端计算,不依赖后端)
|行|内容|
|----|------|
|359-391|全部事件计数逻辑|
|362-376|`getTheme(desc)`按描述第一句关键词打主题标签|
|378-383|按时间排序→相邻帧主题不同=新事件|
|448|`eventCount`填入副标题|
...
|行|内容|
|----|------|
|352|`r.frames`取`report`API的frames数组|
...
## 5.叙事正文:
**后端memory.js**`/api/memory/narrative`路由
|行|内容|
|----|------|
|529-550|从SQLite取所有描述第一句→每50帧一组→写`/tmp/narr_input.txt`→调`reportsupports/scripts/NarrFifty.py`→gemma4:e2b(借100的Ollama,`192.168.0.100:11434`)→各组生成→融合→返回`{narrative}`|
**前端stmem.html**
|行|内容|
|----|------|
...
|行|内容|
|----|------|
|488|mood路由内,用11个地点词正则匹配全部描述文本,出现最多的=`topPlace`|
...
|行|内容|
|----|------|
|488|mood路由内,动态提取人/动物(颜色+衣物+性别),出现最多的=`topPerson`,加量词(一只/一位/一个)|
...
|行|内容|
|----|------|
|322|从全量descriptions统计`kwList`词频,公式`cnt/totalFrames*40`,上限28px下限8px,<20%过滤|
**前端stmem.html**
|行|内容|
|----|------|
|412-417|遍历`r.keywords`,解析`font-size`,限制8-28px|
...
|行|内容|
|----|------|
|315-318|`topPlaces`按`sceneWords`匹配描述第一句→按帧数排序取前8,合并"办公"→"办公室"|
|315|已知问题:词表方式,LLM描述可能匹配不到新地点词汇|
**前端stmem.html**
|行|内容|
|----|------|
...
|385|`people=chars.map(...)`→`{initial,name,count,sub:'最近X月X日',bg}`|
**前端stmem.html**
|行|内容|
|----|------|
|429-433|遍历`r.people`渲染人物卡片(MW风格:40px头像+名字+sub+计数)|
|471|people列表在places-grid右侧|
...
const CLS_ZH = {
  ...
  'outlet': '插座', 'power adapter': '电源适配器', 'hat': '帽子',
};
...
function serveCrop(req, res) {
  ...
  const cropPath = path.join(DATA_DIR, 'datazhx', 'stream', 'objects_img_crop', fname);
...
function serveFrame(req, res) {
  ...
  if (fs.existsSync(DATA_DIR)) {
    const dirs = fs.readdirSync(DATA_DIR).filter(d => d.startsWith('batch_') || d === 'datazhx').sort().reverse();
    ...
    if (db) {
      try {
        ...
        for (let ri = 0; ri < rows.length; ri++) {
          const r = rows[ri];
          const p = palettes[ri % palettes.length];
          const place = clsZh((r.frame_path || 'unknown').replace(/^.*?\//, ''));
          const cropId = r.crop_id || ri;
          items.push({
            place: place,
            time: r.capture_time || '',
            count: 1,
            duration: '--',
            desc: r.description || '',
            bg: 'url(/crops/' + cropId + '.jpg) center/cover',
            bgSoft: p.bgSoft,
            badge: '',
            title: place,
            sub: (r.description || '').slice(0, 60),
            initial: place[0] || '?',
            isRect: true,
            isRound: false,
            tags: [Object.assign({ label: place }, p.tag)],
          });
        }
      } catch (_) {}
    }

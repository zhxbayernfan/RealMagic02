...
scenes = ["room0", "room1", "room2", "room3", "room4", "room5", "office1_static1", "office1_static2", "office1_dynamic1", "office1_dynamic2", "office2_static1", "office2_static2", "office2_dynamic1", "office2_dynamic2"]
...
config = dict(
    ...
    lang=dict(
        ...
        sam_model_path='./models/sam2.1_b.pt',
        ...
        classes_file="./configs/mydata/office1_classes.txt",
        bg_classes=["wall", "floor", "ceiling", "person", "window", "closet_wall", "door", "blackboard", "whiteboard", "closet door"],
        skip_bg=False,
        # 检测过滤参数
        mask_area_threshold=500,             # Mask面积最小阈值（像素），低于则过滤
        max_bbox_area_ratio=0.6,            # 边界框最大面积占比（60%），超过则过滤
        mask_conf_threshold=0.25,            # yolo最小置信度，低于过滤
        box_overlap_threshold=0.95,             # IOU阈值，高于则合并框
        # 合并参数

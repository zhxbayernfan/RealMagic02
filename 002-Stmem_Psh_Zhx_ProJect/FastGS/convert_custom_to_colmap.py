...
def main():
    ...
    for idx, frame_name in enumerate(tqdm(frame_names)):
        ...
        if os.path.exists(point_path):
            ...
            pt_h, pt_w = pts.shape[0], pts.shape[1]

            # Flatten points first, then filter
            pts = pts.reshape(-1, 3)
            ...
            if len(pts) > 0:
                ...
                rgb_img = np.array(Image.open(src_img))  # (H, W, 3)
                # Resize image to match point cloud dimensions (point clouds may
                # be at lower resolution than the original RGB images)
                img_h, img_w = rgb_img.shape[:2]
                if img_h != pt_h or img_w != pt_w:
                    rgb_img = np.array(
                        Image.open(src_img).resize((pt_w, pt_h), Image.BILINEAR)
                    )
                # Flatten RGB and apply same valid mask
                rgb_flat = rgb_img.reshape(-1, 3)
                rgb_sub = rgb_flat[valid]

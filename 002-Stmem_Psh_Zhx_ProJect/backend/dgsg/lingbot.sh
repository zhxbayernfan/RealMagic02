...
# ===================== 执行任务并分别计时 =====================
echo "【1/4】执行 dgsg_refactor.py ..."
t1_start=$(date +%s)
python scripts/dgsg_refactor.py configs/mydata/lingbot.py
t1_end=$(date +%s)

echo "【2/4】执行 construct_scene_graph.py ..."
t2_start=$(date +%s)
python scripts/construct_scene_graph.py configs/mydata/lingbot.py
t2_end=$(date +%s)

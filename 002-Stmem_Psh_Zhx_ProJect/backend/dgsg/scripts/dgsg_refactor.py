...
MIN_MASK_AREA = 2000
MAX_POINTS_PER_OBJECT = 100000 # 体素下采样阈值点
...
def compute_similarity_matrix(detections, projected_masks, objects):
    ...
    for i, det in enumerate(detections):
        for j, obj_id in enumerate(obj_ids):    
            ...
            iou = compute_iou(det['mask'], proj_mask)
...
def fuse_update_matched(objects, detections, match_result):
    ...
    for det_idx, obj_id in match_result['matched_pairs']:
        ...
        # object点数控制：超过阈值时进行体素下采样 (2cm³)
        # if len(obj.points_3d) > MAX_POINTS_PER_OBJECT:
        #     original_len = len(obj.points_3d)
        #     obj.points_3d = voxel_downsample_points(obj.points_3d, voxel_size=VOXEL_SIZE)
        #     print(f"  [DOWNSAMPLE] Obj[{obj.id}]: {original_len} -> {len(obj.points_3d)} points (voxel_size=2cm)")
        ...
        obj.nums += 1
        n = obj.nums
        obj.clip_feature = ((n - 1) * obj.clip_feature + det['clip_feature']) / n
        # 更新物体最佳crop
...
def merge_nearby_objects(objects, center_threshold=0.3, clip_threshold=0.7):
    ...
    for i in range(len(objects)):
        ...
        for j in range(i + 1, len(objects)):
            ...
            # CLIP 特征按观测次数加权平均
            total_nums = primary.nums + absorbed.nums
            primary.clip_feature = (primary.nums * primary.clip_feature + absorbed.nums * absorbed.clip_feature) / total_nums
            primary.nums = total_nums
...
def remove_stale_objects(objects, match_result, rgb_image, depth_image,
    ...
    2) 视锥剔除 + 投影，按像素聚合选择“每像素最近点”（近似 z-buffer）
    ...
    for obj in match_result['unmatched_objects']:
        obj_points = obj.points_3d

        if len(obj_points) < 20:
            print(f"  [DELETE] Obj[{obj.id}] ({obj.class_name}): del_obj_points_nums={len(obj_points)} < 20 -> DELETE")
            objects_to_remove.append(obj)
            continue

        # 视锥剔除与投影
        pts_cam, pts_2d, valid_mask = frustum_culling_gpu(obj_points[:, :3], R, T, intrinsics,
                                                           rgb_image.shape[1], rgb_image.shape[0], device)

        # 像素坐标限制
        pts_2d = np.clip(np.rint(pts_2d).astype(np.int32),
                         [0, 0], [rgb_image.shape[1]-1, rgb_image.shape[0]-1])
        pts_depth = pts_cam[:, 2]
        ...
        # 新增拦截逻辑：如果投影到当前视角并具有有效深度的像素点过少（比如少于20个像素），
        # 往往是只扫到了一个极小的边缘或者全是噪点，或者物体。
        ...
        if obj.id == 11:
            print(f"  visible_num={visible_num}, projected_points_num={projected_points_num}, conflict_num={conflict_num}")
            print(f"  [DELETE] Obj[{obj.id}] ({obj.class_name}): frustum={frustum_ratio:.2f}, visibility={visibility_ratio:.2f}, conflict={conflict_ratio:.3f}")
        ...
        if should_hard_delete:
            ...
            # 只有在可见度大于等于软删除可见度阈值时（或者这里使用硬删除的可见度阈值作为标准），才进行软删除
            ...
            if visibility_ratio >= visibility_ratio_thr:
                ...
                if soft_delete_num > 0:
                    ...
                    remove_nearest_set = set(remove_idx_near.tolist())
                    if len(remove_nearest_set) > 0:
                        all_idx = np.arange(len(obj_points), dtype=np.int64)
                        keep_idx = np.array([i for i in all_idx if i not in remove_nearest_set], dtype=np.int64)
                        
                        obj.points_3d = obj.points_3d[keep_idx]
...
def calculate_view_score(mask_area, xyxy, img_width, img_height):
    ...
    综合四个维度：面积适中度 + 中心度 + 边缘完整性 + 掩码紧密度。

    最佳视角的判断依据：
    - 距离适中：物体占画面 10%~30%（不太远看得清细节，不太近保留轮廓）
    - 构图完整：完整在画面内，四边都有安全距离
    - 位置合理：靠近画面中心
    - 未被遮挡：SAM mask 填满 bbox，形状完整
    """
    image_area = img_width * img_height
    area_ratio = mask_area / image_area
    x_min, y_min, x_max, y_max = xyxy

    # 1. 面积得分 — 高斯钟形曲线，峰值在 17.5%（最佳观察距离）
    #    太远（area_ratio → 0）看不清特征，太近（area_ratio → 1）丢失轮廓
    area_peak = 0.175
    area_sigma = 0.20
    area_score = np.exp(-0.5 * ((area_ratio - area_peak) / area_sigma) ** 2)

    # 2. 中心度得分 — L2 归一化距离，比 max(dist_x, dist_y) 更平滑
    bbox_center_x = (x_min + x_max) / 2.0
    bbox_center_y = (y_min + y_max) / 2.0
    img_center_x = img_width / 2.0
    img_center_y = img_height / 2.0
    dist_norm = np.sqrt(
        ((bbox_center_x - img_center_x) / img_center_x) ** 2 +
        ((bbox_center_y - img_center_y) / img_center_y) ** 2
    ) / np.sqrt(2)  # 对角线最大值归一化到 [0,1]
    center_score = 1.0 - dist_norm

    # 3. 边缘得分 — 连续衰减替代二值判断
    #    计算 bbox 四条边到图像边界的归一化距离，取最小值（最危险的那条边）
    edge_distances = np.array([
        x_min / img_width,                   # 左边
        y_min / img_height,                  # 上边
        (img_width - x_max) / img_width,     # 右边
        (img_height - y_max) / img_height,   # 下边
    ])
    min_edge = np.min(edge_distances)
    # 距离边缘 ≥5% 画面尺寸 → 满分；<5% → 线性衰减到 0
    edge_score = min(1.0, min_edge / 0.05)

    # 4. 掩码紧密度 — mask 面积 / bbox 面积
    #    反映遮挡程度和形状完整度：ratio 低 = 不规则/被遮挡，ratio 高 = 形状完整
    bbox_area = (x_max - x_min) * (y_max - y_min)
    compactness = mask_area / bbox_area if bbox_area > 0 else 0.0

    # 加权和
    w_area, w_center, w_edge, w_compact = 0.30, 0.15, 0.25, 0.30
    score = (
        w_area * area_score +
...
def add_lb_points_to_global_cloud(world_points, rgb_image_hwc, global_cloud,
    ...
    n = len(pts_world)
    if n > 0:
        global_cloud[current_count:current_count + n, :3] = pts_world
        global_cloud[current_count:current_count + n, 3:6] = pts_rgb

...
def get_pointcloud(depth, intrinsics, w2c, sampled_indices):
    ...
    depth_z = depth[0, sampled_indices[:, 0], sampled_indices[:, 1]]  # 1 x h x w 其中 hw=1600
    ...
    A = torch.abs(torch.round(pts, decimals=4))  # 保留4位小数，取绝对值
    B = torch.zeros((1, 3)).cuda().float()
    _, idx, counts = torch.cat([A, B], dim=0).unique(
        dim=0, return_inverse=True, return_counts=True)
    origin = torch.where(counts.gt(1))[0]
    mask = torch.isin(idx, origin)

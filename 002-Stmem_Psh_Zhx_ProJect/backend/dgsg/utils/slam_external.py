...
def remove_points(to_remove, params, variables, optimizer, ins=True):
    # TODO: 该成idx能用的
    to_keep = ~to_remove
    keys = [k for k in params.keys() if k not in ['object_idx', 'cam_unnorm_rots', 'cam_trans']]
    for k in keys:
        group = [g for g in optimizer.param_groups if g['name'] == k][0]
        stored_state = optimizer.state.get(group['params'][0], None)
        if stored_state is not None:
    ...
    if ins:
        params['object_idx'] = params['object_idx'][to_keep.cpu()]
...
def prune_gaussians(params, variables, optimizer, iter, prune_dict):
    ...
    if iter <= prune_dict['stop_after']:
        if (iter >= prune_dict['start_after']) and (iter % prune_dict['prune_every'] == 0):
            ...
            has_remove = torch.any(to_remove)
    ...
    return params, variables, has_remove

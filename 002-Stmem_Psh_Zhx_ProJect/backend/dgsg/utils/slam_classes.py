...
class MapObjectList(DetectionList):
    ...
    def to_serializable(self):
        ...
        for obj in self:
            s_obj_dict= {}
            s_obj_dict['idx'] = obj['idx']
            if 'class_id' in obj:
                s_obj_dict['class_id'] = obj['class_id']
            if 'mask' in obj:
                s_obj_dict['mask'] = obj['mask']
            if 'mask_area' in obj:
                s_obj_dict['mask_area'] = obj['mask_area']
            if 'clip_ft' in obj:
                s_obj_dict['clip_ft'] = to_numpy(obj['clip_ft'])
            if 'num_detections' in obj:
                s_obj_dict['num_detections'] = obj['num_detections']
            if 'image_crops' in obj:
                s_obj_dict['image_crops'] = obj['image_crops']
            if 'caption' in obj:
                s_obj_dict['caption'] = obj['caption']
            if 'description' in obj:
                s_obj_dict['description'] = obj['description']
            if 'category' in obj:
                s_obj_dict['category'] = obj['category']
            if 'best_view_w2c' in obj:
                s_obj_dict['best_view_w2c'] = obj['best_view_w2c']
    ...
    def load_serializable(self, s_obj_list):
        ...
        for s_obj_dict in s_obj_list:
            new_obj = {}
            new_obj['idx'] = s_obj_dict['idx']
            if 'class_id' in s_obj_dict:
                new_obj['class_id'] = s_obj_dict['class_id']
            new_obj['clip_ft'] = s_obj_dict['clip_ft']
            if 'mask' in s_obj_dict:
                new_obj['mask'] = s_obj_dict['mask']
            if 'mask_area' in s_obj_dict:
                new_obj['mask_area'] = s_obj_dict['mask_area']
            if 'num_detections' in s_obj_dict:
                new_obj['num_detections'] = s_obj_dict['num_detections']
            if 'image_crops' in s_obj_dict:
                new_obj['image_crops'] = s_obj_dict['image_crops']
            if 'description' in s_obj_dict:
                new_obj['description'] = s_obj_dict['description']
            if 'category' in s_obj_dict:
                new_obj['category'] = s_obj_dict['category']
            if 'caption' in s_obj_dict:
                new_obj['caption'] = s_obj_dict['caption']
            if 'best_view_w2c' in s_obj_dict:
                new_obj['best_view_w2c'] = s_obj_dict['best_view_w2c']
            self.append(new_obj)

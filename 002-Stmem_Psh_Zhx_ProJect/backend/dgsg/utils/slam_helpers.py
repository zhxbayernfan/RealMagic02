...
def transformed_params_for_detection(params, transformed_gaussians):
    # Check if Gaussians are Isotropic
    if params['log_scales'].shape[1] == 1:
        log_scales = torch.tile(params['log_scales'], (1, 3))
    else:
...
def transformed_params2featurevar(params, transformed_gaussians, curr_vaild_obj_mask):
    # Check if Gaussians are Isotropic
    if params['log_scales'].shape[1] == 1:
        log_scales = torch.tile(params['log_scales'][curr_vaild_obj_mask], (1, 3))
    else:
        log_scales = params['log_scales'][curr_vaild_obj_mask]
    # Initialize Render Variables
    rendervar = {
        'means3D': transformed_gaussians['means3D'][curr_vaild_obj_mask],
        'colors_precomp': params['features'][curr_vaild_obj_mask],
        'rotations': F.normalize(transformed_gaussians['unnorm_rotations'][curr_vaild_obj_mask]),
        'opacities': torch.sigmoid(params['logit_opacities'][curr_vaild_obj_mask]),
        'scales': torch.exp(log_scales),
        'means2D': torch.zeros_like(params['means3D'][curr_vaild_obj_mask], requires_grad=True, device="cuda") + 0
    }
    return rendervar

def transformed_params2featurevar_all(params, transformed_gaussians):
    # Check if Gaussians are Isotropic
    if params['log_scales'].shape[1] == 1:
        log_scales = torch.tile(params['log_scales'], (1, 3))
    else:
...
def transform_to_frame(params, time_idx, gaussians_grad, camera_grad):
    ...
    if camera_grad:
        ...
        cam_rot = F.normalize(params['cam_unnorm_rots'][..., time_idx].detach())   # .detach()以防止梯度在反向传播过程中传播。
